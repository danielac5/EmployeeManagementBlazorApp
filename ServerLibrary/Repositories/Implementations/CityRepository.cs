...
public class CityRepository(AppDbContext appDbContext) : IGenericRepositoryInterface<City>
{
    ...
    {
        if (!await CheckName(item.Name!)) return new GeneralResponse(false, "Raion deja adăugat");
    ...
    private static GeneralResponse NotFound() => new(false, "Raionul nu a fost găsit");

    private static GeneralResponse InsertSuccess() => new(true, "Raion adăugat");
    private static GeneralResponse DeleteSuccess() => new(true, "Raion șters");
    private static GeneralResponse UpdateSuccess() => new(true, "Raion modificat");
