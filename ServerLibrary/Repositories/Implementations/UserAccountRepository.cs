...
public class UserAccountRepository(IOptions<JwtSection> config, AppDbContext appDbContext) : IUserAccount
{
    ...
    public async Task<LoginResponse> SignInAsync(Login user)
    {
        ...
        if (getRoleName is null || string.IsNullOrEmpty(getRoleName.Name)) return new LoginResponse(false, "User role not found or role name is null");

        // verifica daca getRoleName.Name este null
        if (string.IsNullOrEmpty(getRoleName.Name))
        {
            return new LoginResponse(false, "User role name is null");
        }
    ...
    private string GenerateToken(ApplicationUser user, string role)
    {
        ...
        {
            throw new ArgumentNullException(nameof(config.Value.Key)!, "JWT Key cannot be null");
        }
    ...
    public async Task<LoginResponse> RefreshTokenAsync(RefreshToken token)
    {
        ...
        if (updateRefreshToken is null) return new LoginResponse(false, "Refresh token could not generated because user has not signed in");
